object FrmMovInventario: TFrmMovInventario
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsNone
  Caption = 'Movimiento Inventarios'
  ClientHeight = 680
  ClientWidth = 800
  Color = clBtnFace
  DefaultMonitor = dmMainForm
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poOwnerFormCenter
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlSuperior: TPanel
    Left = 0
    Top = 0
    Width = 800
    Height = 57
    Align = alTop
    TabOrder = 0
    object cxLabel3: TcxLabel
      Left = 0
      Top = 7
      AutoSize = False
      Caption = 'MOVIMIENTOS INVENTARIO  '
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -23
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'Metropolis'
      Style.IsFontAssigned = True
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'Metropolis'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'Metropolis'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'Metropolis'
      Properties.Alignment.Horz = taRightJustify
      Properties.LabelEffect = cxleCool
      Properties.LabelStyle = cxlsRaised
      Height = 44
      Width = 800
      AnchorX = 800
    end
    object DateIni: TcxDateEdit
      Left = 50
      Top = 13
      ParentFont = False
      Properties.ButtonGlyph.Data = {
        
        000865574ABF897663FF655749C2000000100000000800000002}
      Properties.DateButtons = [btnClear, btnToday]
      Properties.DisplayFormat = 'dd/mm/yyyy'
      Properties.EditFormat = 'dd/mm/yyyy'
      Properties.SaveTime = False
      Properties.ShowTime = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
      TabOrder = 0
      Width = 115
    end
    object DateFin: TcxDateEdit
      Left = 216
      Top = 13
      ParentFont = False
      Properties.ButtonGlyph.Data = {
        
        000865574ABF897663FF655749C2000000100000000800000002}
      Properties.DateButtons = [btnClear, btnToday]
      Properties.DisplayFormat = 'dd/mm/yyyy'
      Properties.EditFormat = 'dd/mm/yyyy'
      Properties.SaveTime = False
      Properties.ShowTime = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
      TabOrder = 1
      Width = 115
    end
    object cxLabel1: TcxLabel
      Left = 6
      Top = 15
      Caption = 'desde'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
    end
    object cxLabel2: TcxLabel
      Left = 173
      Top = 15
      Caption = 'hasta'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.IsFontAssigned = True
    end
    object BtnBuscar: TcxButton
      Left = 336
      Top = 9
      Width = 93
      Height = 30
      Hint = 'Buscar Documentos'
      ParentCustomHint = False
      BiDiMode = bdLeftToRight
      Caption = 'Buscar'
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'LondonLiquidSky'
      OptionsImage.ImageIndex = 5
      OptionsImage.Images = DmGlobal.ImlBotones
      ParentBiDiMode = False
      ParentShowHint = False
      PopupAlignment = paRight
      ShowHint = True
      TabOrder = 4
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = BtnBuscarClick
    end
  end
  object GrdMovInv: TcxGrid
    Left = 0
    Top = 57
    Width = 800
    Height = 623
    Align = alClient
    TabOrder = 1
    LookAndFeel.Kind = lfStandard
    LookAndFeel.NativeStyle = False
    LookAndFeel.SkinName = 'LondonLiquidSky'
    ExplicitLeft = -8
    ExplicitTop = 63
    object TvlMovInv: TcxGridDBTableView
      Navigator.Buttons.ConfirmDelete = True
      Navigator.Buttons.CustomButtons = <>
      Navigator.Buttons.Images = DmGlobal.ImLNavegador32
      Navigator.Buttons.First.Enabled = False
      Navigator.Buttons.First.Hint = 'Primero'
      Navigator.Buttons.First.ImageIndex = 0
      Navigator.Buttons.First.Visible = False
      Navigator.Buttons.PriorPage.Enabled = False
      Navigator.Buttons.PriorPage.Hint = 'Pagina Anterior'
      Navigator.Buttons.PriorPage.Visible = False
      Navigator.Buttons.Prior.Hint = 'Anterior'
      Navigator.Buttons.Prior.ImageIndex = 2
      Navigator.Buttons.Prior.Visible = True
      Navigator.Buttons.Next.Hint = 'Siguiente'
      Navigator.Buttons.Next.ImageIndex = 3
      Navigator.Buttons.NextPage.Enabled = False
      Navigator.Buttons.NextPage.Hint = 'Pagina siguiente'
      Navigator.Buttons.NextPage.Visible = False
      Navigator.Buttons.Last.Enabled = False
      Navigator.Buttons.Last.Hint = #218'ltimo'
      Navigator.Buttons.Last.ImageIndex = 5
      Navigator.Buttons.Last.Visible = False
      Navigator.Buttons.Insert.Enabled = False
      Navigator.Buttons.Insert.Hint = 'Agregar'
      Navigator.Buttons.Insert.Visible = False
      Navigator.Buttons.Append.Hint = 'Agregar'
      Navigator.Buttons.Append.ImageIndex = 6
      Navigator.Buttons.Append.Visible = True
      Navigator.Buttons.Delete.Hint = 'Borrar'
      Navigator.Buttons.Delete.ImageIndex = 13
      Navigator.Buttons.Delete.Visible = True
      Navigator.Buttons.Edit.Hint = 'Editar'
      Navigator.Buttons.Edit.ImageIndex = 8
      Navigator.Buttons.Edit.Visible = True
      Navigator.Buttons.Post.Hint = 'Guardar'
      Navigator.Buttons.Post.ImageIndex = 9
      Navigator.Buttons.Post.Visible = True
      Navigator.Buttons.Cancel.Hint = 'Cancelar'
      Navigator.Buttons.Cancel.ImageIndex = 12
      Navigator.Buttons.Cancel.Visible = True
      Navigator.Buttons.Refresh.Hint = 'Refrescar'
      Navigator.Buttons.Refresh.ImageIndex = 10
      Navigator.Buttons.Refresh.Visible = True
      Navigator.Buttons.SaveBookmark.Enabled = False
      Navigator.Buttons.SaveBookmark.Visible = False
      Navigator.Buttons.GotoBookmark.Enabled = False
      Navigator.Buttons.GotoBookmark.Visible = False
      Navigator.Buttons.Filter.Enabled = False
      Navigator.Buttons.Filter.Hint = 'Filtrar'
      Navigator.Buttons.Filter.Visible = False
      Navigator.InfoPanel.Visible = True
      DataController.DataSource = DtsMovInv
      DataController.Summary.DefaultGroupSummaryItems = <
        item
          Format = 'Saldo=0.00;Saldo=-0.00'
          Kind = skSum
          Position = spFooter
          FieldName = 'MVI_CANTIDAD'
          Column = TvlMovInvMVI_CANTIDAD
          DisplayText = 'Saldo:'
        end>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      Filtering.ColumnPopup.MultiSelect = False
      NewItemRow.InfoText = 'Clic aqui para agregar un item nuevo'
      OptionsBehavior.FocusCellOnTab = True
      OptionsBehavior.GoToNextCellOnEnter = True
      OptionsBehavior.IncSearch = True
      OptionsBehavior.NavigatorHints = True
      OptionsBehavior.FocusCellOnCycle = True
      OptionsBehavior.ImmediateEditor = False
      OptionsCustomize.ColumnsQuickCustomization = True
      OptionsCustomize.GroupBySorting = True
      OptionsCustomize.GroupRowSizing = True
      OptionsData.Deleting = False
      OptionsData.DeletingConfirmation = False
      OptionsData.Editing = False
      OptionsData.Inserting = False
      OptionsView.NoDataToDisplayInfoText = '<No hay datos para mostrar>'
      OptionsView.ShowEditButtons = gsebAlways
      OptionsView.ColumnAutoWidth = True
      OptionsView.Footer = True
      OptionsView.GroupFooters = gfVisibleWhenExpanded
      OptionsView.HeaderEndEllipsis = True
      OptionsView.Indicator = True
      Styles.Content = DmGlobal.ContenidoLetra
      Styles.Header = DmGlobal.Titulos
      object TvlMovInvMVI_CLASE_DOCU: TcxGridDBColumn
        DataBinding.FieldName = 'MVI_CLASE_DOCU'
        Visible = False
        HeaderAlignmentHorz = taCenter
        VisibleForCustomization = False
        VisibleForEditForm = bFalse
      end
      object TvlMovInvCLASE: TcxGridDBColumn
        Caption = 'Clase'
        DataBinding.FieldName = 'CLASE'
        HeaderAlignmentHorz = taCenter
        Width = 46
      end
      object TvlMovInvMVI_FECHA_DOCU: TcxGridDBColumn
        Caption = 'Fecha Doc.'
        DataBinding.FieldName = 'MVI_FECHA_DOCU'
        PropertiesClassName = 'TcxDateEditProperties'
        Properties.DateButtons = []
        Properties.DisplayFormat = 'dd/mm/yyyy'
        Properties.EditFormat = 'dd/mm/yyyy'
        Properties.InputKind = ikRegExpr
        Properties.ReadOnly = False
        Properties.SaveTime = False
        Properties.ShowTime = False
        Properties.ShowToday = False
        Properties.UseLeftAlignmentOnEditing = False
        Properties.ValidateOnEnter = False
        Properties.YearsInMonthList = False
        HeaderAlignmentHorz = taCenter
        Options.ShowEditButtons = isebNever
        Width = 103
      end
      object TvlMovInvMVI_NUM_DOCU: TcxGridDBColumn
        Caption = 'N'#250'm. Documento'
        DataBinding.FieldName = 'MVI_NUM_DOCU'
        HeaderAlignmentHorz = taCenter
        Width = 114
      end
      object TvlMovInvPRO_CODI: TcxGridDBColumn
        Caption = 'C'#243'd. Producto'
        DataBinding.FieldName = 'PRO_CODI'
        Visible = False
        HeaderAlignmentHorz = taCenter
        Width = 99
      end
      object TvlMovInvPRO_NOMBRE: TcxGridDBColumn
        Caption = 'Producto - Servicio'
        DataBinding.FieldName = 'PRO_NOMBRE'
        HeaderAlignmentHorz = taCenter
        Width = 288
      end
      object TvlMovInvMVI_CANTIDAD: TcxGridDBColumn
        Caption = 'Cantidad'
        DataBinding.FieldName = 'MVI_CANTIDAD'
        PropertiesClassName = 'TcxCurrencyEditProperties'
        Properties.DisplayFormat = ' ,0.;-,0.'
        OnCustomDrawCell = TvlMovInvMVI_CANTIDADCustomDrawCell
        HeaderAlignmentHorz = taCenter
        Width = 71
      end
      object TvlMovInvBOD_CODI: TcxGridDBColumn
        Caption = 'C'#243'd Bodega'
        DataBinding.FieldName = 'BOD_CODI'
        Visible = False
        HeaderAlignmentHorz = taCenter
      end
      object TvlMovInvBOD_NOMBRE: TcxGridDBColumn
        Caption = 'Bodega'
        DataBinding.FieldName = 'BOD_NOMBRE'
        HeaderAlignmentHorz = taCenter
        Width = 63
      end
      object TvlMovInvEMP_CODI: TcxGridDBColumn
        Caption = 'C'#243'd. Empleado'
        DataBinding.FieldName = 'EMP_CODI'
        Visible = False
        HeaderAlignmentHorz = taCenter
        Width = 92
      end
      object TvlMovInvEMP_NOMBRE: TcxGridDBColumn
        Caption = 'Empleado'
        DataBinding.FieldName = 'EMP_NOMBRE'
        HeaderAlignmentHorz = taCenter
      end
      object TvlMovInvMVI_FECHA_SIST: TcxGridDBColumn
        Caption = 'Fecha Sistema'
        DataBinding.FieldName = 'MVI_FECHA_SIST'
        Visible = False
        HeaderAlignmentHorz = taCenter
        Options.ShowEditButtons = isebNever
      end
    end
    object LvlMovInv: TcxGridLevel
      GridView = TvlMovInv
    end
  end
  object QryMovInv: TIBQuery
    Database = DmGlobal.DbGlobal
    Transaction = DmGlobal.TraBase
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'select m.mvi_clase_docu,'
      'CASE'
      '  WHEN mvi_clase_docu = 1 THEN '#39'Salida'#39
      '  WHEN mvi_clase_docu = 0 THEN '#39'Entrada'#39
      'END clase,'
      
        'm.mvi_fecha_docu, m.mvi_num_docu, m.pro_codi, productos.pro_nomb' +
        're,'
      
        'm.mvi_cantidad, m.bod_codi, bodegas.bod_nombre, m.mvi_fecha_sist' +
        ', m.emp_codi,'
      'empleados.emp_nombre'
      'from movinventario m'
      'inner join productos on productos.pro_codi = m.pro_codi'
      'inner join bodegas on bodegas.bod_codi = m.bod_codi'
      'left join empleados on empleados.emp_codi = m.emp_codi'
      'where  m.mvi_fecha_docu between :fechaIni and :fechaFin'
      'order by m.mvi_fecha_docu')
    Left = 608
    Top = 16
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'fechaIni'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'fechaFin'
        ParamType = ptUnknown
      end>
    object QryMovInvMVI_CLASE_DOCU: TIntegerField
      FieldName = 'MVI_CLASE_DOCU'
      Origin = '"MOVINVENTARIO"."MVI_CLASE_DOCU"'
      Required = True
    end
    object QryMovInvCLASE: TIBStringField
      FieldName = 'CLASE'
      ProviderFlags = []
      FixedChar = True
      Size = 7
    end
    object QryMovInvMVI_FECHA_DOCU: TDateTimeField
      FieldName = 'MVI_FECHA_DOCU'
      Origin = '"MOVINVENTARIO"."MVI_FECHA_DOCU"'
      Required = True
    end
    object QryMovInvMVI_NUM_DOCU: TIBStringField
      FieldName = 'MVI_NUM_DOCU'
      Origin = '"MOVINVENTARIO"."MVI_NUM_DOCU"'
      Required = True
      Size = 40
    end
    object QryMovInvPRO_CODI: TIntegerField
      FieldName = 'PRO_CODI'
      Origin = '"MOVINVENTARIO"."PRO_CODI"'
      Required = True
    end
    object QryMovInvPRO_NOMBRE: TIBStringField
      FieldName = 'PRO_NOMBRE'
      Origin = '"PRODUCTOS"."PRO_NOMBRE"'
      Required = True
      Size = 50
    end
    object QryMovInvMVI_CANTIDAD: TIBBCDField
      FieldName = 'MVI_CANTIDAD'
      Origin = '"MOVINVENTARIO"."MVI_CANTIDAD"'
      Precision = 18
      Size = 2
    end
    object QryMovInvBOD_CODI: TIntegerField
      FieldName = 'BOD_CODI'
      Origin = '"MOVINVENTARIO"."BOD_CODI"'
    end
    object QryMovInvBOD_NOMBRE: TIBStringField
      FieldName = 'BOD_NOMBRE'
      Origin = '"BODEGAS"."BOD_NOMBRE"'
      Required = True
      Size = 50
    end
    object QryMovInvMVI_FECHA_SIST: TDateTimeField
      FieldName = 'MVI_FECHA_SIST'
      Origin = '"MOVINVENTARIO"."MVI_FECHA_SIST"'
      Required = True
    end
    object QryMovInvEMP_CODI: TIntegerField
      FieldName = 'EMP_CODI'
      Origin = '"MOVINVENTARIO"."EMP_CODI"'
      Required = True
    end
    object QryMovInvEMP_NOMBRE: TIBStringField
      FieldName = 'EMP_NOMBRE'
      Origin = '"EMPLEADOS"."EMP_NOMBRE"'
      Size = 60
    end
  end
  object DtsMovInv: TDataSource
    DataSet = QryMovInv
    Left = 680
    Top = 24
  end
  object popGrdMovInv: TcxGridPopupMenu
    Grid = GrdMovInv
    PopupMenus = <>
    Left = 544
    Top = 16
  end
end
